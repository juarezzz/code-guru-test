...
/* ---------- Interfaces ---------- */
interface GetMrfScansOvertimeInput {
  mrf_id: string;
  end_date: Date;
  start_date: Date;
...
type MrfScansOvertime = Record<string, Scan[]>;

type LookupTable = Record<string, Record<string, number>>;

/* ---------- Function ---------- */
const get_mrf_scans_overtime = async ({
  mrf_id,
  end_date,
  start_date,
  timestamp_offset,
}: GetMrfScansOvertimeInput) => {
  /* ----------
  ...
  const mrf_scans_overtime: MrfScansOvertime = {};
  ...
  do {
    ...
    try {
      ...
      Rows?.forEach(row => {
        ...
        if (!lookup_table[date]) lookup_table[date] = {};

        if (lookup_table[date][gtin]) lookup_table[date][gtin] += Number(count);
        else lookup_table[date][gtin] = Number(count);
      });
      ...
      if (error_message.endsWith('does not exist')) {
        return { mrf_scans_overtime, bucket };
      }
  ...
  const all_gtins_list = Array.from(
    Object.values(lookup_table).reduce((result: Set<string>, current) => {
      Object.keys(current).forEach(gtin => result.add(gtin));

      return result;
    }, new Set<string>()),
  );

  for (const date in lookup_table) {
    if (Object.prototype.hasOwnProperty.call(lookup_table, date)) {
      const scans = lookup_table[date];

      if (!mrf_scans_overtime[date]?.length) mrf_scans_overtime[date] = [];

      all_gtins_list.forEach(gtin => {
        if (scans[gtin] !== undefined)
          mrf_scans_overtime[date].push({
            gtin,
            count: scans[gtin],
          });
        else
