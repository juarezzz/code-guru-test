...
export class AdminFrontendDeployPipeline extends Construct {
  ...
  constructor(scope: Construct, id: string, props: Props) {
    ...
    this.roles = new PipelineRoles(
      this,
      `AdminFrontendDeploy-Roles-${props.environment}`,
      {
    ...
    this.pipeline_project = new PipelineProject(
      scope,
      `AdminFrontendDeploy-PipelineProject-${props.environment}`,
      {
        projectName: `AdminFrontendDeploy-Pipeline-${props.environment}`,
        ...
        logging: {
          cloudWatch: {
            logGroup: new LogGroup(
              this,
              `AdminFrontendDeploy-Log-Group-${props.environment}`,
              {
          ...
          phases: {
            ...
            build: {
              ...
              commands: [
                ...
                `npx cdk deploy --all -c choice=ADMINWEB-$target_environment -c pipeline=true --require-approval never`,
              ],
            },
            ...
            post_build: {
              ...
              commands: [
                ...
                aws lambda invoke \
                  ...
                  --payload '{"success": '"$success"', "stack": "Admin-Frontend", "environment": "'"$target_environment"'"}' \
                  response.json
            `,
              ],
            },
    ...
    this.pipeline = new Pipeline(
      scope,
      `AdminFrontendDeploy-Pipeline-${props.environment}`,
      {
        pipelineName: `Polytag-AdminFrontendDeploy-${props.environment}`,
    ...
    Tags.of(this.pipeline).add('Custom:Pipeline', 'Admin Frontend');
    Tags.of(this.pipeline).add('Custom:Environment', props.environment);

    Tags.of(this.pipeline_project).add('Custom:Service', 'Codebuild');
    Tags.of(this.pipeline_project).add('Custom:Pipeline', 'Admin Frontend');
