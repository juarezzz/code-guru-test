...
export class PrinterLabelsLambda extends Construct {
  ...
  constructor(scope: Construct, id: string, props: Props) {
    ...
    this.function = new NodejsFunction(
      scope,
      `Printer-Labels-Function-${props.environment}`,
      {
        bundling: {
        ...
        environment: {
          ...
          RESOLVER_DOMAIN: resolver_domain_name,
          DEAD_LETTER_QUEUE_URL:
            props.sqs_construct.dead_letter_queues.printer_dead_letter_queue
              .queueUrl,
        },
        events: [props.sqs_construct.event_sources.printer_event_source],
        functionName: `printer-labels-${props.environment.toLowerCase()}`,
        handler: 'handler',
        initialPolicy: [
          new PolicyStatement({
    ...
    Tags.of(this.function).add('Custom:Event', 'Printer Labels');
