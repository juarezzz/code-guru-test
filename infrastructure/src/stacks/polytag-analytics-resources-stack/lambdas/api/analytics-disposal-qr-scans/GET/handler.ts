...
/* ---------- Modules ---------- */
import { get_disposal_qr_scans_overtime } from '_modules/analytics/functions/get/get-disposal-qr-scans-over-time';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    const { disposal_qr_scans_overtime, bucket } =
      await get_disposal_qr_scans_overtime({
        end_date,
        start_date,
        timestamp_offset,
        gtins: gtins_array,
    ...
    return http_response({
      body: {
        bucket,
        disposal_qr_scans_overtime,
      },
      status_code: 200,
    });
  } catch (error) {
    console.error('Error at GET /analytics-disposal/qr-scans');
    console.error(error);
