...
/* ---------- Modules ---------- */
import { get_average_time_on_landing_page } from '_modules/analytics/functions/get-average-time-on-landing-page';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
     ...
     * ---------- */
    const {
      average_time_on_landing_page: average_time_on_landing_page_result,
    ...
    const average_time_on_landing_page_sum =
      average_time_on_landing_page_result.reduce((acc, val) => acc + val, 0);

    const average_time_on_landing_page = Number(
      Number(
        average_time_on_landing_page_sum /
          average_time_on_landing_page_result.length /
          1000,
      ).toFixed(3),
    );
    ...
    return http_response({
      body: {
        average_time_on_landing_page,
      },
      status_code: 200,
    });
  } catch (error) {
    console.error('Error at GET /analytics-reach/average-time-on-landing-page');
    console.error(error);
