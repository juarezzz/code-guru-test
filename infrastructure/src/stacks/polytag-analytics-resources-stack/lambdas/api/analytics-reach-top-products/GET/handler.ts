...
/* ---------- Modules ---------- */
import { get_top_products } from '_modules/analytics/functions/get/get-top-products';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    const { top_products } = await get_top_products({
      gtins: gtin_array,
    ...
    const formatted_top_products = top_products.map(top_product => {
      return {
        product_id: `brand-product#${top_product.gtin}`,
        product_count: top_product.gtin_count,
      };
    });
    ...
    console.error('Error at GET /analytics-reach/top-products');
    console.error(error);
