...
/* ---------- Modules ---------- */
import { get_product_attributes } from '_modules/product-attributes/functions/get/get-product-attributes';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    if (!roles[group]['brand-product-attributes'].includes('GET'))
      throw new Error(
        handle_http_error({
    ...
    const { product_attributes } = await get_product_attributes({ brand_id });
    ...
    console.error('Error at GET /brand-product-attributes');
    console.error(error);
