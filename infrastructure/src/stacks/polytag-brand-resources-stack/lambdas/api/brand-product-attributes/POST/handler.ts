...
/* ---------- Modules ---------- */
import { create_product_attributes } from '_modules/product-attributes/functions/create/create-product-attributes';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    if (!roles[group]['brand-product-attributes'].includes('POST'))
      throw new Error(
        handle_http_error({
    ...
    const { product_attributes } = await create_product_attributes({
      sub,
      brand_id,
      attributes,
    });
    ...
    console.error('Error at POST /brand-product-attributes');
    console.error(error);
