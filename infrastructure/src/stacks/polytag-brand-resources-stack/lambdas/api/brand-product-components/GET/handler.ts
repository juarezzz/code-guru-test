...
/* ---------- Modules ---------- */
import { get_product_components } from '_modules/product-components/functions/get/get-product-components';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    if (!roles[group]['brand-product-components'].includes('GET'))
      throw new Error(
        handle_http_error({
    ...
    const { product_components } = await get_product_components({ brand_id });
    ...
    console.error('Error at GET /brand-product-components');
    console.error(error);
