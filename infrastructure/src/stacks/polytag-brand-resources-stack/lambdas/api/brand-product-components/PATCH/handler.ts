...
/* ---------- Modules ---------- */
import { update_product_components } from '_modules/product-components/functions/update/update-product-components';
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    if (!roles[group]['brand-product-components'].includes('PATCH'))
      throw new Error(
        handle_http_error({
    ...
    const updated_product_components = await update_product_components({
      sub,
      brand_id,
      components,
    });
    ...
    return http_response({
      body: { updated_product_components },
      status_code: 201,
    });
  } catch (error) {
    console.error('Error at PATCH /brand-product-components');
    console.error(error);
