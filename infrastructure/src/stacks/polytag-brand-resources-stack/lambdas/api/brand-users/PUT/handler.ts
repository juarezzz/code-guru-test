...
const ALLOWED_ROLES = ['brand-admin', 'brand-editor', 'brand-analyst'];
...
export const handler: APIGatewayProxyHandlerV2 = async (
  ...
  try {
    ...
    if (!roles?.[group]?.['brand-users']?.includes('PUT') || !brand_id) {
      throw new Error(
        handle_http_error({
    ...
    const [{ Attributes: brand_user }] = await Promise.all([
    ...
    console.error('Error at PUT /brand-users');
    console.error(error);
